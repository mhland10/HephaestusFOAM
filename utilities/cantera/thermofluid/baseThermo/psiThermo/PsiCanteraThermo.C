...
template<class BaseThermo>
void Foam::PsiCanteraThermo<BaseThermo>::facePropertyCalculate
(
...
) const
{
    ...
    forAll(pf, facei)
    {
        ...
        scalarField Y(Yif.size());

        forAll(Yif, specieI)
        {
            Y[specieI] = Yif[specieI][facei];
        }
        ...
        std::string canteraComposition = printCanteraMixture(Y, mixtureObj);
        ...
        if (propertyName == "rho")
        {
            value = thermo->density();
        }
        else if (propertyName == "he")
        {
            if (useEnthalpy_)
            {
                value = thermo->enthalpy_mass();
                thermo->setState_TPY(298.15, p, canteraComposition);
                value -= thermo->enthalpy_mass();
            }
            else
            {
                value = thermo->intEnergy_mass();
                thermo->setState_TPY(298.15, p, canteraComposition);
                value -= thermo->intEnergy_mass();
            }
        }
        ...
        {
            value = sqrt((thermo->cp_mass()/thermo->cv_mass())*(p/thermo->density()));
        }
        else if (propertyName == "Cp")
        {
            value = thermo->cp_mass();
        }
        else if (propertyName == "Cv")
        {
            value = thermo->cv_mass();
        }
        else if (propertyName == "psi")
        {
            value = thermo->density()/p;
        }
        else if (propertyName == "rPsi")
        {
            value = p/thermo->density();
        }
        else if (propertyName == "gamma")
        {
            value = (thermo->cp_mass()/thermo->cv_mass());
        }
        else
        {
